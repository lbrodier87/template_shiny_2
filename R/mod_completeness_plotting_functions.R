...
#'
#' @export
vis_dat_data_type_simpl <- function(x,
                    sort_type = TRUE,
                    palette = "default",
  ...
  x = x %>% mutate_if(is.POSIXt, as_date)
  
  if(
    !all(unique(unlist(purrr::map(x, class))) %in% 
         c("Date", "factor", "integer", "numeric")) 
  ) {
    stop('The data frame you would like to display contains data type that is currently not handled by the function vis_dat_data_type_simpl().')
  }
  ...
  d <- x %>%
    ...
    dplyr::mutate(value = visdat:::vis_extract_value_(x),
                  valueType = ifelse(is.na(valueType), "Missing", valueType),
                  valueType = forcats::fct_relevel(valueType, "Missing", after = Inf))
  
  # do the plotting
  # vis_dat_plot <-
    # add the boilerplate
    visdat:::vis_create_(d) +
    ...
    ggplot2::scale_fill_manual(
      values = c(RColorBrewer::brewer.pal(n=length(unique(d$valueType))-1, name = "Set1"), "#333333")
      ) +
    ggplot2::theme(axis.text.x = ggplot2::element_text(hjust = 0))
  
  # specify a palette ----------------------------------------------------------
  # visdat:::add_vis_dat_pal(vis_dat_plot, palette)
